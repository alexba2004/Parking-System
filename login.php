...
<?php
    ...
    if($stmt=$conexion->prepare('SELECT id_empleado, email, usuario, contrasena FROM empleados WHERE usuario=?')){
    ...
    $stmt->store_result();
    if($stmt->num_rows>0){
        $stmt->bind_result($id_empleado, $email, $usuario, $contrasena);
        $stmt->fetch();
